...
group 'www-data' do
  ...
  members [username]
  action :modify
  notifies :restart, 'systemd_unit[gemstash.socket]'
end
...
template "#{systemd_dir}/gemstash.socket" do
  source 'gemstash.socket.erb'
  owner username
  group groupname
  mode 0o600
  variables userhome: userhome, clonedir: clonedir
  notifies :run, 'execute[daemon-reload-user]', :immediately
  notifies :stop, 'systemd_unit[gemstash.service]'
  notifies :restart, 'systemd_unit[gemstash.socket]'
end

template "#{systemd_dir}/gemstash.service" do
  source 'gemstash.service.erb'
  owner username
  group groupname
  mode 0o600
  variables userhome: userhome, clonedir: clonedir
  ...
  notifies :run, 'execute[daemon-reload-user]', :immediately
  notifies :enable, 'systemd_unit[gemstash.socket]'
  notifies :restart, 'systemd_unit[gemstash.socket]'
end
